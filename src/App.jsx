  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", color: "#1a2330", background: CINZA_CLARO, minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 1080, margin: "0 auto", padding: "22px 18px 80px" }}>
        {aba === "dados" && <AbaDados dados={dados} setD={setD} setTexto={setTexto} salvarAndamento={salvarAndamento} setStatus={setStatus} />}
        {aba === "itens" && (
...
function ConsultaCliente() {
  ...
  const pesquisar = async () => {
    ...
    if (!termo) { setErro("Digite seu nome ou CPF para consultar."); return; }
    setErro(""); setBuscando(true); setResultados(null);
    ...
    try {
      ...
      const termoNome = normalizarTexto(termo);
      ...
      for (const k of chaves) {
        try {
          ...
          const bateCpf = termoDigitos.length >= 6 && soDigitos(reg.cpf) === termoDigitos;
          const bateNome = termoNome.length >= 3 && normalizarTexto(reg.nome).includes(termoNome);
          if (bateCpf || bateNome) achados.push(reg);
        } catch { /* ignora registro corrompido */ }
      }
      setResultados(achados);
      if (achados.length === 0) setErro("Nenhum atendimento encontrado com esse nome ou CPF. Confira os dados ou fale com a gente pelo WhatsApp.");
    } catch {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Card icon={Search} titulo="Consulte o andamento do seu atendimento">
        <p style={{ fontSize: 13.5, color: "#5a6a80", marginTop: 0 }}>
          Digite seu nome completo ou CPF (com ou sem pontuação) para ver em que etapa está sua vistoria ou laudo.
        </p>
        ...
        {resultados && resultados.length > 0 && (
          <div style={{ display: "grid", gap: 10, marginTop: 16 }}>
            {resultados.map((r, i) => {
              ...
              return (
                <div key={i} style={{ border: `1px solid ${CINZA_BORDA}`, borderRadius: 12, padding: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                    <strong style={{ fontSize: 15 }}>{r.nome || "Cliente"}</strong>
                    <div style={{ flex: 1 }} />
                    <span style={{ background: sc.bg, color: sc.cor, padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{r.status}</span>
                  </div>
                  <div style={{ fontSize: 13, color: "#5a6a80", marginTop: 6 }}>
                    {r.empreendimento && <>Empreendimento: <strong>{r.empreendimento}</strong>{r.unidade && ` · ${r.unidade}`}<br /></>}
                    Atualizado em {fmtData(r.atualizadoEm)}
                  </div>
                </div>
              );
            })}
      ...
      <Card icon={Instagram} titulo="Fale com a FN Edificações">
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <a href="https://wa.me/5581983061305" target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", color: AZUL_MARINHO, background: CINZA_CLARO, padding: "10px 16px", borderRadius: 10, fontSize: 14, fontWeight: 600 }}>
            <MessageCircle size={16} color={AZUL_MEDIO} /> (81) 98306-1305
          </a>
        </div>
      </Card>
    </div>
...
/* ================= Aba: Dados ================= */
function AbaDados({ dados, setD, setTexto, salvarAndamento, setStatus }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card icon={RefreshCw} titulo="Andamento do atendimento (visível ao cliente)">
        <Grid>
          <div style={cell()}>
        ...
        <p style={{ fontSize: 11.5, color: "#8593a8", marginTop: 8, marginBottom: 0 }}>
          O cliente consulta esse status na aba "Consulta do cliente", buscando pelo nome ou CPF informados acima. Requer nome ou CPF preenchido.
        </p>
      </Card>
